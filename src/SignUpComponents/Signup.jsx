...
const Signup = () => {
	...
	const handleSubmit = async (e) => {
		...
		if (!passwordPattern.test(password)) {
			alert(
				"Password must be at least 8 characters long and contain at least one uppercase letter, one lowercase letter, one special character, and one number (without any spaces)."
			);
			return;
		}
		...
		try {
			...
			const response = await axios.post(
				"https://aiorthoscanbackend-production.up.railway.app/api/auth/signup",
				{
					fullName,
					age,
					gender,
		...
		<div className="w-[100vw] min-h-screen bg-background-blue flex flex-col items-center justify-center gap-[20px]">
			...
			<div className="bg-login flex flex-col gap-[5px] px-[20px] items-center justify-center py-[10px] rounded-[10px]">
				...
				<form className="flex flex-col gap-[10px]" onSubmit={handleSubmit}>
					...
					<Link to="/" className="link text-blue-500">
