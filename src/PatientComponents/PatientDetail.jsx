		...
		const fetchPatientData = async () => {
			...
			try {
				const response = await axios.get(
					"https://aiorthoscanbackend-production.up.railway.app/api/main/patientRecords",
			...
			<div className="name-age-gender flex justify-between max-960:grid max-960:grid-cols-2 max-960:gap-[20px] max-960:justify-items-center">
				<div className="patient-box">
					<img
						src={Name}
						alt="Name Icon"
						className="w-[40px] max-400:w-[30px]"
					/>
					<span className="text-[20px] max-900:text-[22px] max-400:text-[16px]">
						{patientData.fullName}
					</span>
					<span className="text-[12px] text-gray-400 max-400:text-[10px]">
						Name
					</span>
				</div>
				...
				<div className="patient-box">
					<img
						src={Profile}
						alt="Profile Icon"
						className="w-[30px] max-400:w-[23px]"
					/>
					<span className="text-[20px] max-900:text-[22px] max-400:text-[16px]">
						{patientData.gender}
					</span>
